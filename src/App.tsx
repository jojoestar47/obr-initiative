...
/**
 * Sort icon with two arrows (up + down).
 * The "active" direction is emphasized; the other is dimmed.
 */
function SortIcon({
  desc,
  className = "",
...
export default function App() {
  ...
  async function selectTokenForCombatant(c: Combatant) {
    // Only token-backed rows should select something
    if (!c.tokenId) return;
    try {
      await OBR.player.select([c.tokenId]);
    } catch {
      // ignore selection errors (e.g., token no longer exists)
    }
  }
  ...
  async function sortByInitiative() {
    ...
    await commit({
      ...state,
      combatants: sorted,
      activeIndex: 0, // reset to top after sorting
    });

    setSortDesc((v) => !v); // toggle direction for next click
  }
          ...
          <header className="p-3 border-b border-zinc-800 flex items-center justify-between gap-2">
            ...
            <div className="flex gap-2 shrink-0 items-center">
              <button
                ...
                disabled={!ready || state.combatants.length === 0}
          ...
          <main className="flex-1 overflow-auto p-3">
            {!ready ? (
              ...
              <div className="text-zinc-400 text-sm">
                No combatants yet. Select tokens, then click{" "}
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="min-w-0">
                          <input
                            onClick={(e) => e.stopPropagation()}
                            className="w-full min-w-0 rounded-xl bg-zinc-900 border border-zinc-800 px-2.5 py-2 text-sm"
                            value={c.name}
                        ...
                        <div>
                          <input
                            onClick={(e) => e.stopPropagation()}
                            className="w-full rounded-xl bg-zinc-900 border border-zinc-800 px-2 py-2 text-sm text-center"
                            type="number"
                            value={c.initiative}
                            onChange={(e) =>
